...
public class TestDao extends Dao {
	...
	private boolean save(Test test, Connection connection) throws Exception {
		...
		try {
			// データベースからテスト情報を取得
			Test old = get(test.getStudent(), test.getSubjet(), test.getSchool(), test.getNo());
			if (old == null) {
				// 学生が存在しなかった場合
				...
				statement.setString(2, test.getSubjet().getCd());
				...
				statement.setString(3, test.getSubjet().getCd());
